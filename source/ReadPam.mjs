...
const readRefErase = (reader, currentFrame) => {
    ...
    let checker = new OutChecker();
    let k = 1;
    for(let refCount = reader.ReadByte(); k <= refCount; k++) {
        let fragmentId = reader.ReadByte();  //计数用id，与对应位图的ref_id一致
        let outLevel = checker.check(fragmentId);
...
const readScript = (reader, currentFrame) => {
     ...
     if(scrKey === 0x01 || scrKey === 0x02) {
         currentFrame.FSCmd = [];
         while(keep) {
             let cmdLength = reader.ReadInt16();  //双字节记录字符串长度
...
const readTransformBlock = (reader, frameNum, isMainAnims) => {
    ...
    for(; j < frameNum; j++) {
        ...
        if(key === 0x01) {
            const k = readRefErase(reader, currentFrame);  //共有k个消除元素
            for(let i = 0; i < k; i++) {  //需要新加k个空帧
                frames.push({'frameIdx': ++j+1});
            }
            if(k < 3) {
                reader.__offest__ += 2;
        ...
        if(canReadTrans) {
            ...
            for(let l = 0; l < transCount; l++) {
                ...
                default:
                    ...
                    throw `Can not identify this type：0x${type.toString(16)}；oldType：0x${oldType.toString(16)}；outLevel：${checker.outLevel}；id：${transform.idx}；transCount: ${transCount}；L：${l}`;
                }
...
export default class ReadPam {
    ...
    readSubAnims(reader, subAnims) {
        ...
        for(let i = 0; i < subAnimCount; i++) {
            let nameLength = reader.ReadInt16();
            let sub = {
                'name': reader.ReadBytes(nameLength).toString().replace('Duplicate Items Folder/', ''),
            };
